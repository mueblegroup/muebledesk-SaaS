<x-superadmin-layout>
    ...
    <x-slot name="header">
        <div>
            ...
            <h1 class="mt-1 text-2xl font-black text-slate-950 dark:text-white">Platform users</h1>
        </div>
    </x-slot>

    <div class="space-y-6">
        <p class="text-sm text-slate-500 dark:text-slate-400">View and manage every account across all companies.</p>
        ...
        <div class="grid gap-6 xl:grid-cols-[380px_minmax(0,1fr)]">
            <form method="POST" action="{{ route('superadmin.users.store') }}" class="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                @csrf
                <div><h2 class="text-lg font-extrabold text-slate-950 dark:text-white">Create user</h2><p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Add any platform or company-level account.</p></div>
                ...
                <div><x-input-label for="role" value="System role" /><select id="role" name="role" class="mt-1 block w-full" required>@foreach ($roles as $role)<option value="{{ $role->value }}" @selected(old('role') === $role->value)>{{ str($role->value)->headline() }}</option>@endforeach</select></div>
                <div><x-input-label for="company_ids" value="Companies" /><select id="company_ids" name="company_ids[]" multiple class="mt-1 block min-h-32 w-full">@foreach ($companies as $company)<option value="{{ $company->id }}" @selected(in_array($company->id, old('company_ids', [])))>{{ $company->name }}</option>@endforeach</select><p class="mt-1 text-xs text-slate-400">Not required for superadmins. Hold Ctrl/Cmd to select multiple.</p></div>
                <div><x-input-label for="membership_role" value="Company membership role" /><select id="membership_role" name="membership_role" class="mt-1 block w-full">@foreach (['member' => 'Member', 'admin' => 'Company admin', 'owner' => 'Owner'] as $value => $label)<option value="{{ $value }}" @selected(old('membership_role', 'member') === $value)>{{ $label }}</option>@endforeach</select></div>
