<x-superadmin-layout>
    ...
    <x-slot name="header"><div><h1 class="text-2xl font-extrabold text-slate-950 dark:text-white">Subscription plans</h1><p class="mt-1 text-sm text-slate-500 dark:text-slate-400">Fixed pricing, flexible duration, company and role limits, server-enforced feature entitlements, and safe upgrade ordering.</p></div></x-slot>
    ...
    <div class="grid gap-6 xl:grid-cols-[430px_minmax(0,1fr)]">
        <form method="POST" action="{{ route('superadmin.plans.store') }}" class="space-y-4 rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
            ...
            <div><x-input-label for="company_limit" value="Companies per account"/><x-text-input id="company_limit" name="company_limit" type="number" min="1" class="mt-1 block w-full" :value="old('company_limit')" placeholder="Unlimited"/><p class="mt-1 text-xs text-slate-500">Leave empty for unlimited company creation. The highest active allowance across companies owned by the account is used.</p></div>
            ...
            @forelse ($plans as $plan)
                ...
                <article class="rounded-3xl border border-slate-200 bg-white p-6 shadow-sm dark:border-slate-800 dark:bg-slate-900">
                    <form method="POST" action="{{ route('superadmin.plans.update', $plan) }}" class="space-y-4">@csrf @method('PUT')
                        ...
                        <div class="grid gap-3 md:grid-cols-4"><x-text-input name="company_limit" type="number" min="1" :value="$plan->company_limit" placeholder="Unlimited companies"/><x-text-input name="admin_limit" type="number" min="0" :value="$plan->admin_limit" placeholder="Unlimited admins"/><x-text-input name="employee_limit" type="number" min="0" :value="$plan->employee_limit" placeholder="Unlimited employees"/><x-text-input name="client_limit" type="number" min="0" :value="$plan->client_limit" placeholder="Unlimited clients"/></div>
