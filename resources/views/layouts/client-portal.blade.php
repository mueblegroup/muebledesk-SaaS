    ...
    <aside class="fixed inset-y-0 left-0 z-50 flex w-72 -translate-x-full flex-col border-r border-white/10 bg-slate-950 text-white shadow-2xl transition duration-300 lg:sticky lg:top-0 lg:h-screen lg:translate-x-0" :class="sidebarOpen ? 'translate-x-0' : '-translate-x-full lg:translate-x-0'">
        <div class="border-b border-white/10 px-5 py-5">
            <a href="{{ route('client-portal.dashboard') }}" class="flex items-center gap-3">
                <span class="flex h-12 w-12 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-500 to-violet-600 text-xl font-black shadow-lg shadow-indigo-500/30">M</span>
        ...
        <nav class="flex-1 space-y-1 overflow-y-auto px-4 py-5">
            ...
            @endforeach
            @if ($billingCompany && Route::has('client-portal.billing.show'))
                <a href="{{ route('client-portal.billing.show', $billingCompany) }}" class="flex items-center gap-3 rounded-2xl px-4 py-3 text-sm font-bold transition {{ request()->routeIs('client-portal.billing.*') ? 'bg-white text-slate-950 shadow-lg shadow-black/10' : 'text-slate-300 hover:bg-white/10 hover:text-white' }}">
    ...
    <div class="min-w-0 flex-1">
        <header class="sticky top-0 z-30 border-b border-slate-200/80 bg-white/85 backdrop-blur-xl">
            <div class="flex items-center justify-between gap-4 px-4 py-4 sm:px-6 lg:px-10">
                <div class="flex items-center gap-3">
                    <button type="button" class="rounded-2xl border border-slate-200 bg-white p-3 text-slate-700 shadow-sm lg:hidden" @click="sidebarOpen = true" aria-label="Open navigation"><svg class="h-5 w-5" fill="none" stroke="currentColor" viewBox="0 0 24 24"><path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M4 6h16M4 12h16M4 18h16"/></svg></button>
                    <div>@if(isset($header)){{ $header }}@else<h1 class="text-xl font-extrabold tracking-tight">Client Portal</h1><p class="text-sm text-slate-500">Manage your SaaS account and companies</p>@endif</div>
                </div>
        ...
        <main class="min-h-[calc(100vh-76px)] bg-[radial-gradient(circle_at_top_right,_rgba(99,102,241,0.10),_transparent_35%),radial-gradient(circle_at_bottom_left,_rgba(14,165,233,0.08),_transparent_30%)] px-4 py-7 sm:px-6 lg:px-10 lg:py-10">
            <div class="fixed right-4 top-24 z-50 w-full max-w-sm space-y-3">
                @foreach (['success' => 'emerald', 'status' => 'emerald', 'error' => 'red', 'warning' => 'amber'] as $key => $tone)
                    @if(session($key))<div x-data="{show:true}" x-show="show" x-init="setTimeout(()=>show=false,5000)" class="rounded-2xl border bg-white px-4 py-3 text-sm font-bold shadow-xl {{ $tone === 'emerald' ? 'border-emerald-200 text-emerald-700' : ($tone === 'red' ? 'border-red-200 text-red-700' : 'border-amber-200 text-amber-700') }}">{{ session($key) }}</div>@endif
                @endforeach
            </div>
            {{ $slot }}
        </main>
