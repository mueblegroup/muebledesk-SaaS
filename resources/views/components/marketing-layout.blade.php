...
<head>
    ...
    <meta name="description" content="{{ $description ?? 'MuebleDesk helps businesses manage customers, quotations, invoices, payments, expenses, recurring billing and Malaysian e-Invoices in one place.' }}">
...
<div x-data="{ mobileOpen:false }" class="min-h-screen overflow-x-hidden">
    <header class="sticky top-0 z-50 border-b border-white/60 bg-white/85 backdrop-blur-xl">
        <div class="mx-auto flex h-20 max-w-7xl items-center justify-between px-4 sm:px-6 lg:px-8">
            <a href="{{ route('marketing.home') }}" class="flex items-center gap-3">
                ...
                <span><span class="block text-base font-black tracking-tight">MuebleDesk</span><span class="block text-[11px] font-bold uppercase tracking-[.18em] text-slate-400">Invoicing made simple</span></span>
            </a>
            <nav class="hidden items-center gap-7 lg:flex">
                @foreach([['route'=>'marketing.features','label'=>'Features'],['route'=>'marketing.how-it-works','label'=>'How it works'],['route'=>'marketing.security','label'=>'Security'],['route'=>'marketing.pricing','label'=>'Pricing'],['route'=>'marketing.contact','label'=>'Contact']] as $item)
            ...
            <div class="hidden items-center gap-3 lg:flex">
                ...
                <a href="{{ route('register') }}" class="rounded-2xl bg-slate-950 px-5 py-3 text-sm font-extrabold text-white shadow-lg shadow-slate-950/15 transition hover:-translate-y-0.5 hover:bg-indigo-600">Get started</a>
            </div>
            <button @click="mobileOpen=!mobileOpen" class="rounded-xl border border-slate-200 p-2.5 lg:hidden">☰</button>
        </div>
        <div x-show="mobileOpen" x-cloak class="border-t border-slate-100 bg-white p-4 lg:hidden"><div class="grid gap-2">@foreach([['route'=>'marketing.features','label'=>'Features'],['route'=>'marketing.how-it-works','label'=>'How it works'],['route'=>'marketing.security','label'=>'Security'],['route'=>'marketing.pricing','label'=>'Pricing'],['route'=>'marketing.contact','label'=>'Contact'],['route'=>'login','label'=>'Log in'],['route'=>'register','label'=>'Create account']] as $item)<a href="{{ route($item['route']) }}" class="rounded-xl px-4 py-3 text-sm font-bold text-slate-700 hover:bg-slate-50">{{ $item['label'] }}</a>@endforeach</div></div>
    </header>
    {{ $slot }}
    <footer class="border-t border-slate-200 bg-slate-950 text-white">
        ...
        <div class="border-t border-white/10 px-4 py-6 text-center text-xs text-slate-500">© {{ now()->year }} MuebleDesk. Helping businesses invoice with confidence.</div>
    </footer>
</div>
</body>
</html>