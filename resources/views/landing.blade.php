<!DOCTYPE html>
<html lang="{{ str_replace('_', '-', app()->getLocale()) }}" data-force-theme="light" style="color-scheme: light;">
<head>
    <script>
...
<body class="landing-page min-h-screen font-sans antialiased">
    <div class="relative isolate overflow-hidden">
        <div class="absolute inset-x-0 top-0 -z-10 h-[42rem] bg-[radial-gradient(circle_at_top_left,rgba(99,102,241,.18),transparent_38%),radial-gradient(circle_at_top_right,rgba(217,70,239,.12),transparent_32%)]"></div>
        ...
        <header class="landing-header mx-auto flex max-w-7xl items-center justify-between border-b border-transparent px-5 py-6 sm:px-8 lg:px-10">
            <a href="{{ route('landing') }}" class="flex items-center gap-3">
                <span class="flex h-11 w-11 items-center justify-center rounded-2xl bg-gradient-to-br from-indigo-600 via-violet-600 to-fuchsia-500 text-lg font-black text-white shadow-lg shadow-indigo-500/20">M</span>
            ...
            <nav class="flex items-center gap-2 sm:gap-3">
                <a href="{{ route('login') }}" class="rounded-xl px-3 py-2 text-sm font-bold text-slate-600 transition hover:bg-slate-100 hover:text-slate-950 sm:px-4">Log in</a>
                <a href="{{ route('register') }}" class="rounded-xl bg-slate-950 px-4 py-2 text-sm font-black text-white transition hover:bg-indigo-600">Create account</a>
            </nav>
        </header>
        ...
        <main>
            <section class="mx-auto grid max-w-7xl gap-12 px-5 pb-24 pt-14 sm:px-8 lg:grid-cols-[1.05fr_.95fr] lg:items-center lg:px-10 lg:pb-32 lg:pt-20">
                <div>
                    <span class="inline-flex rounded-full border border-indigo-200 bg-indigo-50 px-4 py-2 text-xs font-black uppercase tracking-[.18em] text-indigo-700">Built for growing businesses</span>
                ...
                <div class="relative">
                    <div class="absolute -inset-8 -z-10 rounded-[3rem] bg-indigo-500/15 blur-3xl"></div>
                    <div class="landing-surface rounded-[2rem] border p-4 backdrop-blur-xl sm:p-6">
                        <div class="landing-soft-surface rounded-[1.5rem] border p-5 sm:p-7">
                            <div class="flex items-center justify-between gap-4"><div><p class="text-sm font-black">MuebleDesk Client Portal</p><p class="landing-subtle mt-1 text-xs">Choose a company workspace</p></div><span class="rounded-full bg-emerald-100 px-3 py-1 text-xs font-black text-emerald-700">Account ready</span></div>
            ...
            <section id="how-it-works" class="landing-section-tint border-y">
                <div class="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-10">
                    <div class="max-w-2xl"><p class="text-xs font-black uppercase tracking-[.2em] text-indigo-600">Simple onboarding</p><h2 class="mt-3 text-3xl font-black sm:text-4xl">Three clear steps from visitor to invoicing workspace.</h2></div>
                    <div class="mt-10 grid gap-5 md:grid-cols-3">
                        @foreach ([
                            ...
                            <article class="landing-surface rounded-3xl border p-6"><span class="text-sm font-black text-indigo-600">{{ $number }}</span><h3 class="mt-5 text-xl font-black">{{ $title }}</h3><p class="landing-muted mt-3 text-sm leading-6">{{ $description }}</p></article>
                        @endforeach
                    </div>
                </div>
            </section>
            ...
            <section class="mx-auto max-w-7xl px-5 py-20 sm:px-8 lg:px-10">
                <div class="grid gap-5 md:grid-cols-2 lg:grid-cols-3">
                    @foreach ([
                        ...
                        <article class="landing-surface rounded-3xl border p-6"><div class="flex h-11 w-11 items-center justify-center rounded-2xl bg-indigo-100 font-black text-indigo-700">✓</div><h3 class="mt-5 text-lg font-black">{{ $title }}</h3><p class="landing-muted mt-2 text-sm leading-6">{{ $description }}</p></article>
                    @endforeach
                </div>
            </section>
