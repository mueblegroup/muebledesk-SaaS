...
class MyInvoisClient
{
    ...
    {
        $environment = app()->bound('currentCompany')
            ? (string) Setting::get('myinvois_environment', 'sandbox')
    ...
    public function isEnabled(): bool
    {
        if (app()->bound('currentCompany')) {
            return filter_var(Setting::get('myinvois_enabled', '0'), FILTER_VALIDATE_BOOL);
        }

        return (bool) config('myinvois.enabled');
    }
    ...
    public function authenticate(bool $forceRefresh = false): array
    {
        ...
        if (empty($response['access_token'])) {
            throw new RuntimeException('MyInvois authentication did not return an access token.');
        }
    ...
    {
        $response = $this->authenticatedRequest()->get($this->apiUrl().'/api/v1.0/taxpayer/validate/'.rawurlencode($tin), [
            'idType' => $idType,
            'idValue' => $idValue,
        ]);
    ...
    {
        return $this->normalise($this->authenticatedRequest()->put($this->apiUrl().'/api/v1.0/documents/state/'.rawurlencode($uuid).'/state', [
            'status' => 'cancelled',
            'reason' => trim($reason),
        ]));
    }
    ...
    {
        if (! $this->isEnabled()) {
            throw new RuntimeException('MyInvois submission is disabled for this company.');
        }

        if ($this->environment() === 'production') {
            if (! app()->environment('production')) {
                throw new RuntimeException('Production MyInvois submissions require APP_ENV=production.');
            }
            if (! config('myinvois.production_enabled')) {
                throw new RuntimeException('Production MyInvois submission is disabled by the platform operator.');
            }
        }
    }
    ...
    private function credentials(): array
    {
        if (app()->bound('currentCompany')) {
            $environment = $this->environment();
            $credentials = [
                'client_id' => (string) Setting::get("myinvois_{$environment}_client_id", ''),
                'client_secret' => (string) Setting::get("myinvois_{$environment}_client_secret", ''),
            ];
        } else {
            $credentials = (array) config('myinvois.environments.'.$this->environment(), []);
        }

        if (empty($credentials['client_id']) || empty($credentials['client_secret'])) {
            throw new RuntimeException('MyInvois credentials are missing for '.$this->environment().'. Configure them in Admin → e-Invoice Setup.');
        }
    ...
    private function tokenCacheKey(): string
    {
        ...
        return config('myinvois.cache.token_key').'.'.$companyId.'.'.$this->environment();
    }

    private function apiUrl(): string
    {
        return rtrim((string) config('myinvois.environments.'.$this->environment().'.api_url'), '/');
    }

    private function portalUrl(): string
    {
        return rtrim((string) config('myinvois.environments.'.$this->environment().'.portal_url'), '/');
    }
}
